...
이 repository 안의 naive trie와 `StaticTransitionMatrixDecoder`는 correctness 확인과 benchmark
비교군을 위한 검증용 코드입니다. 추천 평가와 API serving의 기본 실행 경로는
...
| `static_decoding.decoding_pt.generate_and_apply_logprobs_mask` | inference, benchmark, tests | PyTorch sparse tail candidate gather |
| `static_decoding.decoding_jax.generate_and_apply_logprobs_mask` | benchmark, tests | JAX sparse tail candidate gather |
| `static_decoding.decoding_pt.sparse_transition_torch` | benchmark, tests | upstream PyTorch decoding harness 동작 검증 |
| `static_decoding.decoding_jax.sparse_transition_jax` | benchmark, tests | upstream JAX decoding harness 동작 검증 |
| `static_decoding.decoding_pt.RandomModel` | benchmark, tests | upstream harness용 dummy model |
| `static_decoding.decoding_jax.RandomModel` | benchmark, tests | upstream JAX harness용 dummy model |
...
upstream `sparse_transition_torch` harness는 `model(input_ids[:, -1:])` 형태의 decoder-only
모델 호출을 전제합니다.
...
- full upstream harness 검증: `static_decoding.decoding_pt.sparse_transition_torch`,
...
- upstream PyTorch/JAX harness가 생성한 Semantic ID가 모두 유효한지
...
- upstream benchmark suite의 Hash bitmap, PPV baseline까지 포함한 동일 조건 재현
...
upstream harness를 테스트와 benchmark에서 검증한다. 추천 모델 inference는 프로젝트 모델의
